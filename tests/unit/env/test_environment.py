...
class TestEnvironmentReset:
    ...
    def test_reset_without_tasks_is_internally_terminal(self, empty_corpus) -> None:
        ...
        assert env.episode.done is True
        assert obs.termination_reason == "no_tasks"
...
class TestEnvironmentStep:
    ...
    def test_seeded_reset_is_deterministic(
        self, corpus, tasks: list
    ) -> None:
        """The same seed should select the same task."""
        env_one = SearchEnvironment(corpus=corpus, tasks=tasks)
        env_two = SearchEnvironment(corpus=corpus, tasks=tasks)

        obs_one = env_one.reset(seed=7)
        obs_two = env_two.reset(seed=7)
...
class TestBudgetManagement:
    ...
    def test_budget_warning_at_threshold(self, env_with_config) -> None:
        ...
        assert env.episode.context_token_count > 0
