...
class TestRewardCalculation:
    ...
    def test_supporting_chunk_ids_surface_evidence_metrics(
        self, corpus: DocumentCorpus, tasks: list
    ) -> None:
        """Answer results should include evidence metrics and unsupported ids."""
        env = SearchEnvironment(corpus=corpus, tasks=tasks)
        env.reset()
        search_obs = env.step(SearchAction.make_search("Instagram launched 2010"))
        assert search_obs.action_result is not None
        chunk_id = search_obs.action_result["results"][0]["chunk_id"]
        env.step(SearchAction.make_read([chunk_id]))

        obs = env.step(
            SearchAction.make_answer(
                "2010",
                [chunk_id, "not_a_real_chunk"],
            )
        )

        result = obs.action_result or {}
        assert "evidence_precision" in result
        assert "evidence_recall" in result
        assert result["supporting_chunk_ids_used"] == [chunk_id, "not_a_real_chunk"]
        assert "not_a_real_chunk" in result["invalid_supporting_chunk_ids"]
